...
contract CKPTValStaking is Ownable, AccessControl {
    ...
    // Reward rate by lock time (tokens per second)
    uint256 public rewardRateByLockTime;

    // Reward scaling factor of ckpt signature (default 1.0 ^(-18) in fixed-point, 1e18)
    uint256 public CKPTRewardScalingFactor = 1;
    ...
    struct Checkpoint {
        ...
        uint256 powerSum; // Accumulated voting power
    }
    ...
    constructor(
        ...
        rewardRateByLockTime = _rewardRate;
    ...
    function setUnstakeLockPeriod(uint256 _lockPeriod) external onlyOwner {
        unstakeLockPeriod = _lockPeriod;
    }
    ...
    // Set reward scaling factor (only owner)
    function setCKPTRewardScalingFactor(
        uint256 _CKPTRewardScalingFactor
    ) external onlyOwner {
        require(
            _CKPTRewardScalingFactor > 0,
            "Scaling factor must be greater than 0"
        );
        CKPTRewardScalingFactor = _CKPTRewardScalingFactor;
    }
        ...
        )
    {
        ...
        if (
            ...
            currentRewards =
                currentRewards +
                ((timeElapsed * rewardRateByLockTime * validator.stakedAmount) /
                    1e18);
        }
    ...
    function distributeCheckpointRewards(uint64 _epochNum) external {
        ...
        // Calculate reward per validator with scaling factor applied
        uint256 rewardPerValidator = (checkpoint.powerSum *
            CKPTRewardScalingFactor) / (validatorCount * 1e18);
    ...
    function _getSortedValidators() internal view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < count; i++) {
            uint256 index = (validatorCursor + i) % validatorCount;
            sortedValidators[i] = validatorAddresses[index];
        }
